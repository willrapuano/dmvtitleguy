...
|---|---|---|
| Durable lead-rate, conversion, and idempotency ledger | Complete | Production Turso contains `LeadRateLimitBucket`, write-once `LeadSubmission` acquisition fields, append-only `LeadSubmissionEvent` history, and an encrypted short-lived GHL opportunity outbox. |
...
- Added an additive Production Turso migration for write-once acquisition fields, append-only event history, encrypted GHL recovery, delivery/reconciliation, attribution, qualification, acceptance, close, outcome value, and lost-reason fields. The live database gate verifies every required table and column.
...
- Paused new paid title-search orders and replaced the checkout path with a request-introduction route pending merchant/provider/fulfillment authorization. Both PayPal order creation and capture fail closed with HTTP 503; no payment can be captured through the site.
...
### August 26 adversarial hardening release

- A technical-delivery, SEO-measurement, and entity/authority review panel independently challenged the next release. Its blocking findings were incorporated before promotion: fail-closed payment capture, rendered provider-truth enforcement, an encrypted GHL opportunity outbox, append-only submission events, explicit QA exclusion, production health checks, and deterministic GSC/SpyFu checkpoints.
- The production Turso migration is additive and verified. The GHL recovery worker runs every 15 minutes, retries only the opportunity sync after confirmed webhook delivery, and never replays the lead webhook.
- GHL permits one transaction-pipeline opportunity for the reserved QA contact. The recovery path now reuses that durable contact card while the local ledger retains submission-level history. Controlled submission `89179407-ea8e-4ab8-ba23-62dd7d93c1ec` returned success, an identical replay returned `duplicate: true`, and reconciliation updated the existing card with the exact submission ID, `request-title-review`, Submitted/open state, and `SEO QA Excluded = true`. After verification the card was restored to Lost and retained the QA marker.
- The full release suite passed after the one-card recovery and monitoring corrections: lint, TypeScript, zero high-severity production dependency vulnerabilities, 289-route build, source and 261-route rendered provider-truth gates, lead security/attribution, canonical/sitemap, structured content, internal-link, and CMS-resilience checks.
...
> Hello — your “Types of Property Lines” article currently links to Federal Title’s old `/understanding-4-types-of-property-surveys/` URL, which returns 404. I publish a DMV-focused guide covering boundary, location-drawing, improvement, and ALTA/NSPS survey distinctions at `https://dmvtitleguy.io/blog/types-of-property-surveys-dc-md-va`. If it meets your editorial standards, it may be a useful replacement or additional reference. I’m Will Rapuano, Marketing and Business Development Officer at Pruitt Title; DMV Title Guy is an independently operated educational and business-development website, separate from Pruitt’s corporate site. No compensation or reciprocal link is requested.
...
> Will Rapuano | DMV Title Guy. Practical DC, Maryland, and Virginia title and closing education. Marketing and Business Development Officer at Pruitt Title LLC. DMV Title Guy is independently operated and separate from Pruitt’s corporate site. Eligible transaction requests may be referred to Pruitt for its independent review.
