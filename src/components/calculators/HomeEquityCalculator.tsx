  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="equity-home-value" className={labelClass}>Current Home Value</label>
          <div className="relative"><span aria-hidden="true" className="absolute left-3 top-3.5 text-gray-600 text-sm">$</span>
            <input id="equity-home-value" type="number" placeholder="600,000" value={homeValue} onChange={e => setHomeValue(e.target.value)} className={inputClass + " pl-7"} />
          </div>
        </div>
        <div>
          <label htmlFor="equity-mortgage-balance" className={labelClass}>Remaining Mortgage Balance</label>
          <div className="relative"><span aria-hidden="true" className="absolute left-3 top-3.5 text-gray-600 text-sm">$</span>
            <input id="equity-mortgage-balance" type="number" placeholder="350,000" value={mortgageBalance} onChange={e => setMortgageBalance(e.target.value)} className={inputClass + " pl-7"} />
          </div>
        </div>
        <div>
          <label htmlFor="equity-other-liens" className={labelClass}>Other Liens (HELOC, 2nd mortgage)</label>
          <div className="relative"><span aria-hidden="true" className="absolute left-3 top-3.5 text-gray-600 text-sm">$</span>
            <input id="equity-other-liens" type="number" placeholder="0" value={otherLiens} onChange={e => setOtherLiens(e.target.value)} className={inputClass + " pl-7"} />
          </div>
        </div>
        <div>
          <label htmlFor="equity-ltv-target" className={labelClass}>Max LTV Target (for borrowing)</label>
          <div className="relative">
            <input id="equity-ltv-target" type="number" placeholder="80" value={ltvTarget} onChange={e => setLtvTarget(e.target.value)} className={inputClass + " pr-7"} />
      ...
      <div>
        ...
        {!results ? (
          <div className="surface-subtle p-6 text-center text-brand-muted text-sm">Enter your home value to see equity</div>
        ) : (
